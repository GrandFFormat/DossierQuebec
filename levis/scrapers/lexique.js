...
import { decouperResolutions, lignesUtiles, recoller } from '../lib/pv.js';
...
async function main() {
  ...
  for (const t of TERMES) {
    ...
    const ex = decisions.filter((d) => d.numero && re.test(d.objet ?? '')).sort((a, b) => b.date.localeCompare(a.date))[0] ?? null;
    entrees.push({
      ...t,
  ...
  const payload = {
    ...
    source: 'https://levis.ca/fr/ville/conseil-municipal/seances-du-conseil-municipal/archives-des-seances-du-conseil-municipal',
    avertissement:
      'Les définitions de ce lexique sont rédigées par DossierVille — ce ne sont pas des textes de la ' +
      'Ville de Lévis ni des définitions légales. Ce qui vient des documents officiels, ce sont les ' +
