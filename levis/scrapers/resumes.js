...
//
// À Lévis, le sommaire est un PDF à part (« Document d'aide à la décision FIN-2026-035 »),
// atteint par l'hyperlien que le procès-verbal porte lui-même sur son identifiant (voir
// lib/pv.js, champ `sommairePdf`). On le télécharge, on en extrait le texte (lib/pdf.js), et
// on le garde en cache dans data/textes/ pour ne jamais le redemander à la Ville. Sa première
// page dit la DIRECTION qui l'a rédigé : on la garde comme unité administrative.
//
...
// « DIRECTION FINANCES ET TRÉSORERIE » sur la première page du sommaire. Recopiée telle
// quelle, seulement remise en minuscules après la première lettre (« Finances et trésorerie »).
export function directionDuSommaire(texte) {
  const m = String(texte ?? '').match(/^\s*DIRECTION\s+(.{3,90}?)\s*$/m);
  if (!m) return null;
  const brut = m[1].trim().toLocaleLowerCase('fr-CA');
  return brut.charAt(0).toLocaleUpperCase('fr-CA') + brut.slice(1);
}
...
async function texteSommaire(d) {
  ...
  const data = await telechargerPdf(d.sommairePdf);
  if (!data) return null;
...
async function candidats({ depuis = null, plafond = Infinity, cache }) {
  ...
  for (const d of decisions ?? []) {
    if (!d.sommairePdf || !d.dossier) continue;
  ...
  for (const d of liste) {
    let texte = null;
    try {
      texte = await texteSommaire(d);
    } catch (err) {
      console.warn(`⚠ ${d.dossier} : ${err.message}`);
    }
    if (!texte) {
