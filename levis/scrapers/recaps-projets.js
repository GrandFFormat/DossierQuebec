...
// (quebec/scrapers/details-argent.js, outils partagés dans lib/ia.js) : un contrôle mécanique — chaque numéro cité doit appartenir au
...
import { MODELE, appeler, compacter, nombresDe } from '../lib/ia.js';
...
function dossiersDuProjet(decisions, resumes, cle) {
  ...
  return [...groupes.entries()]
    .map(([k, docs]) => {
      ...
      // Lévis : le dossier est un sommaire et ses résolutions ; on le nomme par sa résolution la
      // plus récente, comme Mes dossiers (scripts/projets-publics.js).
      ...
      return {
        k,
        date: docs.at(-1).date,
        numeros: [...new Set([...docs.map((d) => d.numero), docs[0].sommaireId].filter(Boolean))],
...
// ---------- rédaction ----------
const CONSIGNE = `Tu rédiges, pour des citoyens qui suivent un grand projet de la Ville de Lévis, le
...
const CONSIGNE_VERIFICATION = `Tu contre-vérifies le récapitulatif d'un projet de la Ville de Lévis, rédigé à partir
