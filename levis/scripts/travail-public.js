...
// Lévis : les sommaires ne sont pas des documents de la liste ; on compte les sommaires distincts
// cités par les résolutions, et les dossiers par sommaire.
const parDossier = new Map(docs.filter((d) => d.dossier).map((d) => [d.dossier, d]));
const sommaires = [...parDossier.values()];
const resolutions = docs.filter((d) => d.numero).length;
...
const travail = {
  ...
  annee,
  documents: {
    total: docs.length + sommaires.length,
    resolutions,
    sommaires: sommaires.length,
    procesVerbaux: compter('Procès-verbal'),
    autres: 0,
  },
  parMois,
