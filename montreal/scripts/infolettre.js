// Brouillon du compte rendu mensuel — l'infolettre municipale, volet Montréal.
// Même gabarit que quebec/scripts/infolettre.js ; ce qui change : les instances (le conseil
// municipal et 19 arrondissements), les noms des types de documents, et pas de détail de
// l'argent ni d'agenda (attendues.json), que Montréal ne produit pas encore.
//
...
//   npm run infolettre -- --seance=2026-07-06 --instance=plateau-mont-royal  après une séance d'arrondissement
//
...
const TYPES_MONTANT = {};
const VERSION_VERIFICATION = 0;
const chargerDetails = async () => ({ details: [] });
const extraireDetails = async () => {};
...
const ARRONDISSEMENTS_MTL = [
  ['ahuntsic-cartierville', "d'Ahuntsic-Cartierville", 'Ahuntsic-Cartierville'],
  ['anjou', "d'Anjou", 'Anjou'],
  ['cote-des-neiges-notre-dame-de-grace', 'de Côte-des-Neiges–Notre-Dame-de-Grâce', 'Côte-des-Neiges–Notre-Dame-de-Grâce'],
  ['lachine', 'de Lachine', 'Lachine'],
  ['lasalle', 'de LaSalle', 'LaSalle'],
  ['ile-bizard-sainte-genevieve', "de L'Île-Bizard–Sainte-Geneviève", "L'Île-Bizard–Sainte-Geneviève"],
  ['mercier-hochelaga-maisonneuve', 'de Mercier–Hochelaga-Maisonneuve', 'Mercier–Hochelaga-Maisonneuve'],
  ['montreal-nord', 'de Montréal-Nord', 'Montréal-Nord'],
  ['outremont', "d'Outremont", 'Outremont'],
  ['pierrefonds-roxboro', 'de Pierrefonds-Roxboro', 'Pierrefonds-Roxboro'],
  ['plateau-mont-royal', 'du Plateau-Mont-Royal', 'du Plateau-Mont-Royal'],
  ['riviere-des-prairies-pointe-aux-trembles', 'de Rivière-des-Prairies–Pointe-aux-Trembles', 'Rivière-des-Prairies–Pointe-aux-Trembles'],
  ['rosemont-la-petite-patrie', 'de Rosemont–La Petite-Patrie', 'Rosemont–La Petite-Patrie'],
  ['saint-laurent', 'de Saint-Laurent', 'Saint-Laurent'],
  ['saint-leonard', 'de Saint-Léonard', 'Saint-Léonard'],
  ['sud-ouest', 'du Sud-Ouest', 'du Sud-Ouest'],
  ['verdun', 'de Verdun', 'Verdun'],
  ['ville-marie', 'de Ville-Marie', 'Ville-Marie'],
  ['villeray-saint-michel-parc-extension', 'de Villeray–Saint-Michel–Parc-Extension', 'Villeray–Saint-Michel–Parc-Extension'],
];
...
const INSTANCE_COURTE = (i) =>
  (i ?? '')
    .replace("Conseil d'arrondissement de ", 'Arr. ')
    .replace("Conseil d'arrondissement du ", 'Arr. du ')
    .replace("Conseil d'agglomération", 'Agglomération')
    .trim();
...
async function main() {
  ...
  const titreCourriel = seance ? `Ce que ${seance.sujet} a décidé ${nomPeriode}` : `Ce que la Ville de Montréal a décidé ${nomPeriode}`;
  ...
  const docs = tous.filter((d) => d.type !== 'Procès-verbal' && d.type !== 'Ordre du jour');
  // Une séance : ses résolutions, et les sommaires qu'elles citent (préparés plus tôt), pour le résumé.
  if (seance) {
    if (!tous.some((d) => d.type === 'Résolution')) throw new Error(`Aucune résolution de « ${seance.instance} » le ${seance ? depuis : ''} dans les données.`);
  }
  const nbResolutions = tous.filter((d) => d.type === 'Résolution').length;
  const documentsSeance = tous.filter((d) => d.type === 'Procès-verbal' || d.type === 'Ordre du jour');
  ...
  for (const d of tous) {
    ...
    const i = /^\s*Conseil d'arrondissement/i.test(d.instance) ? "Conseils d'arrondissement" : INSTANCE_COURTE(d.instance);
  ...
  const ORDRE = ['Conseil municipal', 'Agglomération', 'Comité exécutif', "Conseils d'arrondissement"];
  ...
  for (const d of docs) {
    // À Montréal, le dossier suit la décision d'une instance à l'autre : c'est lui qui regroupe.
    const cle = d.dossier ?? d.id;
    ...
    f.dates.push(d.date);
    // Le résumé du sommaire (clé : le dossier) avant celui du texte de la résolution (clé : son numéro).
    const r = (d.sommaireId && resumeParId.get(d.sommaireId)) || resumeParId.get(d.id);
    if (r && (!f.resume || r.source === 'sommaire')) f.resume = r;
  }
  ...
  const sommaireDeResolution = new Map(decisions.decisions.filter((d) => d.type === 'Résolution' && d.sommaireId).map((d) => [d.id, d.sommaireId]));
  ...
  for (const v of votes.votes.filter((v) => dansFenetre(v) && surInstance(v) && v.contre?.length && v.pour?.length)) {
    ...
    const resume = resumeParId.get(sommaireDeResolution.get(v.id)) ?? resumeParId.get(v.id);
  ...
  const introduction = seance
    ? `À sa séance ${nomPeriode.replace(/^le /, 'du ')}, ${seance.sujet} a adopté ${pluriel(nbResolutions, 'résolution', 'résolutions')} sur ${pluriel(liste.length, 'dossier', 'dossiers')}` +
      ...
      (lourdes[0] ? `. Le plus gros montant : ${argent(lourdes[0].montant)} — ${phraseDe(lourdes[0]).replace(/^La Ville (?:de Montréal )?/, 'la Ville ').replace(/\.$/, '')}` : '') +
      '.'
    :
    `${nomPeriode[0].toUpperCase()}${nomPeriode.slice(1)}, ${pluriel(liste.length, 'dossier a', 'dossiers ont')} passé devant les instances de la Ville de Montréal, en ${pluriel(nbSeances, 'séance', 'séances')}` +
    (accordees.length ? `, dont ${pluriel(accordees.length, 'subvention accordée', 'subventions accordées')}${totalSubventions ? ` pour ${argent(totalSubventions)} au total` : ''}` : '') +
    (contrats.length ? ` et ${pluriel(contrats.length, 'contrat', 'contrats')} avec un montant` : '') +
    (lourdes[0] ? `. Le plus gros montant : ${argent(lourdes[0].montant)} — ${phraseDe(lourdes[0]).replace(/^La Ville (?:de Montréal )?/, 'la Ville ').replace(/\.$/, '')}` : '') +
    '.';
  ...
  L.push('---', '');
  L.push(`Les alertes quand vos projets bougent, et le site gratuit pour tout le monde : ${RACINE}/abonnement`, '');
  ...
  H.push(`
    <tr><td style="background:#0B8A4B;border-radius:12px 12px 0 0;padding:26px 24px 22px">
      <div style="${POLICE};font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#CFF5DE">DossierVilleDeMontréal · ${seance ? 'Après la séance' : 'Compte rendu mensuel'}${abonne ? ' · édition abonnés' : vitrine ? ' · édition abonnés, offerte ce mois-ci' : ''}</div>
  ...
  H.push(`<tr><td style="padding:22px 4px 4px">${paragraphes.join('')}<p style="${POLICE};margin:0;font-size:16px;line-height:1.6;color:${ENCRE}">${echapper(introduction)}</p>${abonne
    ...
    // Montréal n'a pas encore d'encadrés dorés (détail de l'argent, agenda) : on dit ce que
    // l'abonnement donne vraiment ici — les alertes —, sans promettre le reste.
    : boiteOr(`${marque} <strong>L'abonnement</strong> : une alerte chaque matin où vos projets, vos mots-clés ou vos organismes bougent, à Montréal comme à Québec — et c'est lui qui garde le site gratuit pour tout le monde. ${lienOr("Voir l'abonnement", `${RACINE}/abonnement`)}`, '14px 0 0')}</td></tr>`);
  ...
  // Montréal n'a pas d'attendues.json : sans dossier en attente, la section n'existe pas.
  if (agendaTotal) H.push(enTete(SECTIONS.agenda, null, marque));
  if (!agendaTotal) { /* rien */ } else if (payant) {
  ...
  if (abonne) H.push(`<tr><td style="padding:26px 0 0">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#E8F6EE;border:1px solid #9FD9B6;border-radius:10px;border-collapse:separate"><tr><td style="${POLICE};padding:16px 18px;font-size:15px;line-height:1.55;color:${ENCRE}">
      ...
      Vos alertes du matin continuent chaque jour où vos projets, vos mots-clés ou vos organismes bougent. Dans <a href="${RACINE}/mes-dossiers" style="color:#0B8A4B;font-weight:700">Mes dossiers</a> : l'export en tableur ou en PDF, et le détail de l'argent de chaque dossier.
