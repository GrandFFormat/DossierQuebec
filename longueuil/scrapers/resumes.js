...
// recommandation, contexte, justification, décisions antérieures, aspects financiers. Le
// « pourquoi » y est déjà écrit — on ne fait que le rendre lisible en trois à sept puces.
//
// À Longueuil, le sommaire est dans le document de séance (« Global ») : scrapers/sommaires.js
// en a extrait le texte, page par page, dans data/textes/sommaire_SD-….json. Ce script ne
// télécharge rien ; un sommaire dont le texte manque attend la prochaine lecture du document.
//
...
const CONSIGNE = `Tu résumes des sommaires décisionnels de la Ville de Longueuil pour un site
...
la Ville s'engage à faire, avec qui elle contracte. Le sommaire se termine par les noms des
personnes qui l'ont rédigé et approuvé : ne les reprends pas. Laisse tomber le vocabulaire de procédure
...
// Les candidats : les résolutions de l'année qui citent un sommaire, une fois par sommaire (un
// même dossier passe par le conseil de ville, qui oriente, puis par le conseil d'agglomération,
// qui décide). Le résumé est indexé par NUMÉRO DE SOMMAIRE : `id` = « SD-2026-0984 », et les
// fiches de décision le retrouvent par `sommaireId`.
//
// Le plafond compte les sommaires DONT ON A LE TEXTE : un sommaire cité au procès-verbal mais
// absent du document de séance ne doit pas occuper une place à chaque exécution.
async function candidats({ depuis = null, plafond = Infinity, cache }) {
  ...
  for (const d of decisions ?? []) {
    if (d.type !== 'Résolution' || !d.sommaireId) continue;
    if (depuis && (d.date ?? '') < depuis) continue;
    if (cache.has(d.sommaireId)) continue;
    if (!parDossier.has(d.sommaireId) || (d.date ?? '') > (parDossier.get(d.sommaireId).date ?? '')) parDossier.set(d.sommaireId, d);
  }
  ...
  for (const d of liste) {
    ...
    docs.push({ id: d.sommaireId, numero: d.sommaireId, resolution: d.numero, date: d.date, unite: sommaire.direction ?? null, objet: sommaire.titre ?? d.objet, pdf: d.sommairePdf, page: sommaire.page ?? null, texte: preparerTexte(sommaire.texte) });
  }
  if (sansTexte) console.log(`${sansTexte} sommaire(s) cité(s) sans texte extrait (absents du document de séance, ou document pas encore lu).`);
  return docs;
}
