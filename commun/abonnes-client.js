...
// ---------- navigation entre les volets et les pages communes ----------
// Un volet mémorise la page où l'on était ; Mes dossiers et Abonnement proposent d'y revenir,
// au lieu de renvoyer à l'accueil du volet. Seulement un chemin du site, jamais une adresse
// extérieure.
...
// L'en-tête des pages communes : le retour au volet quitté, les villes, la taille du texte et
// le thème — les mêmes réglages (clés dvq:zoom et dvq:theme) que dans les volets.
export function enteteCommune() {
  ...
  const cible = volet ?? (VILLES[ville] ? { ville, chemin: `/${ville}/` } : null);
  if (retour && cible) {
  ...
  if (villes) villes.innerHTML = `Villes : ${Object.entries(VILLES).map(([v, nom]) => `<a href="/${v}/">${echapper(nom)}</a>`).join(' · ')}`;
