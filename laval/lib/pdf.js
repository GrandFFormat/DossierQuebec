...
function lignesDePage(items) {
  ...
  return lignes.map(({ y, texte, fragments }) => ({ y, texte, x: fragments[0]?.x ?? 0, fragments: fragments.map(({ x, str }) => ({ x: Math.round(x * 10) / 10, str: str.replace(ESPACE_INSECABLE, ' ') })) }));
}
...
// Page par page, sans tout garder : pour le document de séance de Longueuil (« Global », près de
// mille pages et 94 Mo), dont on ne veut que les pages des sommaires décisionnels. `surPage`
// reçoit { numero, lignes, texte } et peut renvoyer `false` pour arrêter la lecture.
export async function parcourirPdf(data, surPage) {
  const lib = await pdfjs();
  const tache = lib.getDocument({ data, useSystemFonts: true, isEvalSupported: false, disableFontFace: true, verbosity: 0 });
  const doc = await tache.promise;
  const nombrePages = doc.numPages;
  try {
    for (let i = 1; i <= nombrePages; i++) {
      const page = await doc.getPage(i);
      const lignes = lignesDePage((await page.getTextContent()).items);
      page.cleanup();
      if ((await surPage({ numero: i, lignes, texte: lignes.map((l) => l.texte).join('\n') })) === false) break;
    }
  } finally {
    await tache.destroy();
  }
