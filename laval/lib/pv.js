...
export function parserVotes(corps) {
  ...
  const votes = [];
  const reDebut = /demande\s+le\s+vote\s+sur\s+/g;
  const reFin = /\n\s*(?:ADOPT[ÉE]E?|REJET[ÉE]E?)\b|\n\s*\(SD-|demande\s+le\s+vote\s+sur\s+/g;
  let m;
...
function lireVote(passage, avant) {
  ...
  const mDecompte = t.match(/(adopt|rejet|accept)[ée]e?\s+par\s+un\s+compte\s+de\s+(\d+)\s+en\s+faveur\s+et\s+de\s+(\d+)\s+contre\s*:?/i);
  ...
  const objetVote = t.match(/demande\s+le\s+vote\s+sur\s+([^,:]{1,80}?)\s*(?:,|\s+(?:laquelle|lequel)\b)/i)?.[1]?.trim() ?? null;
  const etiquette = objetVote && !/^la\s+proposition$/i.test(objetVote) && !/^celui-ci$|^celle-ci$/i.test(objetVote) ? objetVote.charAt(0).toUpperCase() + objetVote.slice(1) : null;
  ...
  return {
    etiquette,
    resultat: /^rejet/i.test(mDecompte[1]) ? 'Rejetée' : 'Adoptée',
    pour: pour.noms,
    contre: contre.noms,
    decomptePour,
    decompteContre,
...
const CHAPITRE_ODJ = /^(\d{1,2})\.\s+(\S.*)$/;
const POINT_ODJ = /^(\d{1,2}\.\d{1,3})\s+(\S.*)$/;
const RENVOI_ODJ = /^(SD-\d{4}-\d{1,6})(?:\s*-\s*CT\s*:?\s*(\d+))?\s*$/i;
const DISTRICTS_ODJ = /^Districts?\(s\)?\s*:\s*(.*)$/i;
const MONTANT_ODJ = /^Montants?\(s\)?\s*:\s*(.*)$/i;
const HORS_ODJ = /^(?:Service du Greffe|ORDRE DU JOUR|Généré le\b|Page\s+\d+)/i;
...
export function parserOrdreDuJour(texte) {
  const points = [];
  let chapitre = null;
  ...
  for (const brut of normaliserTexte(texte).split('\n')) {
    const l = brut.trim();
    if (!l || HORS_ODJ.test(l)) continue;
    const mp = l.match(POINT_ODJ);
    if (mp) {
      fermer();
      chapitreOuvert = false;
      courant = { numero: mp[1], chapitre: chapitre?.libelle ?? null, chapitreNumero: chapitre?.numero ?? null, morceaux: [mp[2]], sommaire: null, ct: null, districts: [], montant: null };
      continue;
    }
    const mc = l.match(CHAPITRE_ODJ);
...
export function apparierOrdreDuJour(resolutions, points) {
  ...
  for (const r of resolutions) {
    ...
    if (file?.length) resultat.set(r.numero, file.shift());
  }
  return resultat;
}
